...
def get_main_menu():
    btn1 =
    btn2 =
    kb = types.ReplyKeyboardMarkup(keyboard=[btn1, btn2], resize_keyboard=True)
    return kb

@dp.message(Command("start"))
async def cmd_start(message: types.Message):
    await message.answer("Бот JolToo.exp запущен!", reply_markup=get_main_menu())
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post('https://pay.crypt.bot', headers=headers, json=data) as resp:
            ...
            if res.get('ok'):
                ...
                await message.answer(f"Ссылка на оплату 1 USDT: {url}")
            else:
